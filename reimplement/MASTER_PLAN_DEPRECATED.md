# Hospital Radiology Schedule System v2: Master Plan

**Status**: Ready for implementation
**Date**: 2024-11-15 (Updated)
**Approach**: Go rewrite preserving proven v1 patterns
**Timeline**: 3 months to production (+ 1 week v1 security fixes in parallel)
...
- 10× faster Amion scraping (sequential 180s → parallel 2-3s via goroutines)
...
## 17 Critical Decisions (Locked In)
...
- **Choice**: Asynq job library with Redis backend
- **Why**: Battle-tested, built-in retry/monitoring, don't build custom job infrastructure
- **Implementation**:
  - Use `github.com/hibiken/asynq` for async job processing
  - Job types: ODS import, Amion scraping, coverage resolution
  - Long-running tasks return job ID immediately
  - Asynq web UI provides job monitoring/dashboard
  - Job results stored in PostgreSQL, queued in Redis
  - Built-in retry, timeout, priority, scheduled tasks
- **Alternative**: `github.com/RichardKnop/machinery` if Redis unavailable (supports PostgreSQL broker)
- **Performance gain**: 6 months of Amion data: 180s → 2-3s (parallel goroutines)
...
- **Critical tests**:
  ...
  - Job tests: Asynq task execution
...
**Decision 13: ODS File Parsing**
- **Choice**: Go ODS library (github.com/knieriem/odf or similar)
- **Why**: Focus on business logic, not XML parsing
- **Error handling**: Validation errors collected (not fail-fast), same as v1

**Decision 14: Amion Web Scraping**
- **Choice**: Simple HTTP client + goquery (CSS selectors)
- **Why**: If Amion serves HTML directly, 10× faster than browser automation
- **Performance**: 6 months parallel (5 concurrent goroutines) = 2-3 seconds vs v1's 180s
- **Fallback**: If Amion uses heavy JavaScript, switch to headless browser (Chromedp)
...
- **Implementation**:
  ...
  - **Audit logs**: Export v1 audit logs to archive (S3/cold storage) before v1 shutdown
  - **Historical queries**: Optional "historical schedule" API endpoint proxies to v1 for data older than 30 days
...
- **2 people**: v2 Phase 0 (schema study, setup)
...
### Phase 0: Pre-Development (1 week) ✅ UPDATED
...
- Write script: v1 `audit_log` table → JSON export
...
**Day 4: Job Library Selection & Testing**
- **Person**: Test/DevOps Engineer
- Integrate Asynq library in sample Go project
- Test sample scraping job with Redis backend
- Verify Redis compatibility with hospital infrastructure
- Document Asynq job patterns for team
- **Fallback**: Test Machinery if Redis unavailable
...
- [ ] Asynq job library integrated and tested
...
- Integrate Asynq for job processing:
  - Job handlers for ODS import, Amion scraping
  - Job status via Asynq web UI
...
- Implement async endpoints (ODS import, Amion scraping):
  - Return Asynq job ID immediately
...
- ODS import as Asynq job
- Amion scraping as Asynq job
- Job status tracking via Asynq web UI
...
- Performance benchmarks (document 10× improvement)

**Deliverables**:
- Working Amion scraper (2-3 sec for 6 months, 60× faster than v1)
- ODS import with file validation (XXE protected)
- Asynq job system fully operational
...
- Runbooks (how to operate v2, troubleshooting)
...
- **Week 52**: Export v1 audit logs to cold storage (S3)
...
3. **Test & DevOps Engineer** (70% testing, 30% DevOps)
   ...
   - CI/CD pipeline
   - **Week 1**: Asynq job library evaluation and integration
...
- [ ] Asynq job library integrated
- [ ] Docker environment runs locally
- [ ] Team trained on decisions
- **Go/No-Go**: Proceed if v1 is secure AND schema translation complete
...
### Phase 3 Completion
- [ ] Amion scraper working, 2-3 sec for 6 months (vs v1's 180s)
...
- **Go/No-Go**: Proceed if performance goals met

### Phase 4 Completion
- [ ] Test coverage 85%+
- [ ] Monitoring/observability working (Prometheus metrics exported)
- [ ] Documentation complete (runbooks, API docs, schema docs)
...
| **Custom job system too complex** | **MITIGATED**: Use Asynq library (battle-tested, maintained) |
| Amion HTML format changes | Keep Chromedp approach ready (1-2 day switch) |
...
- Amion scraping parallelized (180s → 2-3s via goroutines)
...
- Asynq job system working
- Rate limiting preventing brute force

### Phase 3 Done
- Amion scraper working, performance target met (2-3s for 6 months)
- ODS import with validation (XXE protected)
...
**On v1's timeline** (3 months, 3 people):
...
- 10× performance improvement (Amion scraping: 180s → 2-3s)
...
- Less code to maintain (Asynq vs custom job system, sqlc vs ORM magic)

---

## Next Steps

1. **This week**: Share master plan with team, get approval
2. **Week 1, Day 1**: v1 security fixes (parallel track) → production-ready
3. **Week 1, Day 1-7**: Phase 0 begins (schema translation, setup)
4. **Week 3**: Phase 1 begins (core services)
5. **Week 7**: Phase 2 begins (API, security)
6. **Week 10**: Phase 3 begins (scrapers, integration)
7. **Week 12**: Phase 4 begins (testing, polish)
8. **Week 14**: Cutover week

**Total: 14 weeks to production** (standard full-time, 3 people)
**Critical: Week 1 delivers secure v1** (removes urgency from v2 timeline)
...
**Confidence Level**: High (17 decisions locked, clear roadmap, team allocated, v1 risks mitigated)
**Risk Level**: Low-Medium (v1 security fixed Week 1, schema preserved not redesigned, proven library choices)
**Success Probability**: 90% on time, 98% eventual delivery
**Key Success Factor**: Maintainability over novelty (translate proven patterns, don't reinvent)
